...
bool GameManager::canPlace(int blockIndex)
{
...
int GameManager::checkFullLine(GameState& gameState) {
...
int GameManager::mergeBlock(GameState& gameState) {
	int curBlockX = blocks[curBlock].getX();
	int curBlockY = blocks[curBlock].getY();
	int curBlockAngle = blocks[curBlock].getAngle();
	int curBlockShape = blocks[curBlock].getShape();
...
int GameManager::mergeBlock(GameState& gameState, int blockIndex)
{
	int curBlockX = blocks[blockIndex].getX();
	int curBlockY = blocks[blockIndex].getY();
	int curBlockAngle = blocks[blockIndex].getAngle();
...
int GameManager::moveBlock(GameState& gameState) {
...
int GameManager::moveBlock(GameState& gameState, int blockIndex)
{
...
void GameManager::currentBlockRotate()
{
	if (canRotate()) {
		blocks[curBlock].erase();
...
int GameManager::getCurBlockX() const
{
	return blocks[curBlock].getX();
}

int GameManager::getCurBlockY() const
{
	return blocks[curBlock].getY();
}

int GameManager::getCurBlockIndex() const
{
	return curBlock;
}
...
int GameManager::moveDown(GameState& gameState)
{
	int is_gameover = 0;
...
int GameManager::hardDrop(GameState& gameState)
{
	int is_gameover = 0;
	while (is_gameover == 0)
	{
...
int GameManager::hardDrop(GameState& gameState, int blockIndex, int x, int y)
{
...
void GameManager::addHolds(int count) {
	remainHolds += count;
	showHoldBox(); // 횟수가 올랐으니 화면 즉시 갱신
}

void GameManager::showHoldBox() {
...
void GameManager::holdCurrentBlock() {
...
void GameManager::addRandomLine()
{
	int line[12];
...
void GameManager::drawStageClear() {
...
void GameManager::drawCombo(int count) {
